...
| **놓기 단계는 정책 단독으로 성립하지 않는다** | 수평 이송은 목표 11 mm 까지 도달하나 수직 하강이 일어나지 않아 0.5 m 상공에서 물체를 놓는다. 허용 오차 안에서 놓은 판 0 / 40여 판 |

두 번째와 세 번째가 이 저장소의 주된 결과이며, 모두 음성 결과다.
...
집기와 놓기를 분리해 측정했다. 앞의 절제 실험은 모두 놓기를 끈 상태다.

![놓기 단계](docs/media/place.svg)

| 지표 | 값 |
|---|---|
| 수평 이송 오차 (최선) | 0.011 m |
| 개방 시 물체 높이 | 1.421 m (테이블 0.865, 목표 0.941) |
| 허용 오차 안에서 놓은 판 | 0 / 40여 판 |

목표 11 mm 까지 도달한 판은 그리퍼를 열지 않았고, 그리퍼를 연 판은 모두 0.5 m
이상 상공이었다. **같은 정책이 집기의 하강은 수행한다.**

놓기 목표 검출에는 별도의 결함이 있다 — 검출이 집을 물체 위로 홈잉한 뒤
수행되어, 놓을 자리의 물체가 화각을 벗어나면 다른 물체를 낮은 점수로 채택한다.
자세한 내용과 씬 배치 민감도는 [docs/RESULTS.md §6](docs/RESULTS.md)에 있다.
...
그 밖의 미완성 항목과 남은 반론은 [docs/RESULTS.md §7](docs/RESULTS.md)에 정리했다.
